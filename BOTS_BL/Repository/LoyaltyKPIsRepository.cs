            ...
            try
            {
                if (GroupId == "1326")
                {
                    using (var contextnew = new CommonDBContext())
                    {
                        ...
                        objLoyaltyKPIs = contextnew.Database.SqlQuery<LoyaltyKPIs>("sp_LoyaltyPerfromance @pi_GroupId,@pi_DBName, @pi_Date, @pi_LoginId",
                        new SqlParameter("@pi_GroupId", GroupId),
                        new SqlParameter("@pi_DBName", DBName),
                        new SqlParameter("@pi_Date", DateTime.Now.ToString("yyyy-MM-dd")),
                        new SqlParameter("@pi_LoginId", loginId)).FirstOrDefault<LoyaltyKPIs>();
                    }
                }
